...
~/Library/LaunchAgents/learn.tick.*.plist    the prepare-ahead tick, on macOS, only when the learner turned it on
~/Learning/tick.log, tick.err           the tick's run log (one JSON line per goal it ran) and its diagnostics
~/.config/learn/tick.env                optional credential for the tick's agent (KEY=VALUE, 0600, never in the learning root)
```
...
```
{
  ...
  "prepare_ahead": null          null, or {"command", "time", "leave_notes"}, written by `schedule.py install` (Prepare ahead)
}
```
...
Returning to a goal: read the vault note, the page, and the goal's `answers.json` and `state.json` if they exist. A scheduled run may have graded a lesson and left the note (Prepare ahead): when the page shows a lesson graded or passed that the note does not, bring the note up to date from the page's feedback first (tick the line, add the misconceptions it names) and commit. Grade finished lessons first (below), then the review answers (Review). With nothing to grade, continue from the active lesson: open the page (see Hosting) and say in one line how many review items are due today, if any, then how far the lesson got as answered of total and which question is next ("1 review due first, then lesson 2: 1 of 6 answered, q2 next", or "0 of 6 answered, q1 next" for a lesson not started; `resume` in `state.json` names the last lesson item answered). Never grade a lesson the learner has not finished or said `done` on. The page opens at the resume point and shows its own "back after N days" line; you add nothing for that.
...
- **Calendar,** for a learner who would rather not install an app (run `nudge off` first when ntfy was on): `serve.py ics --root <learning root> --time HH:MM` (`--link <page URL>` for a hosted goal) writes `<learning root>/reminder.ics`, a daily event at that time with an alert. Open it (`open`, `xdg-open`) to import it and say so in one line. Imported events never update: a new time means a new file, and the learner deletes the old event.
...
An opt-in daily job, for served goals only, that grades a finished lesson and writes the next one while the learner is away, so the next lesson is waiting when they sit down. Offer it only when the learner asks for lessons to be ready ahead, or in `settings`. It needs `python` and macOS for now (elsewhere `install` says so, and grading waits for the next session).

- **Turn it on:** `<python> <this skill's folder>/scripts/schedule.py install --root <learning root> --time HH:MM`, a time the laptop is usually on or asleep (not shut down), before the reminder time: 04:00 unless the learner says otherwise. It checks the installed `claude` supports the unattended flags, saves `prepare_ahead` in the profile and loads the job. A Mac asleep at that time runs it on waking. If its output has `warning` (the learning root or notes folder is in Documents, Desktop, iCloud Drive or a cloud folder, where macOS may block the job or wait on an Allow nobody answers), say it in one line and offer `--leave-notes` for the notes folder: the note is then updated at the next interactive run. On `ok: false`, give its error in one line. A goal whose source file sits in one of those folders is still graded ahead, but its next lesson waits for the next interactive run: macOS would hold the job's read of the source on a dialog nobody answers. Say so in the same line when the learner's sources are there.
- **The server.** The job never starts the goal server. When the profile's `phone` is `none`, offer the login job too (`schedule.py install --server`, as in Same Wi-Fi step 2), since after a restart nothing else serves the lesson the job wrote until the next session.
- **Other agents:** `--command '<JSON argv>'` instead, with the placeholders `tick.py` lists; the prompt is added last.
- **Status and off:** `schedule.py status --root <learning root>` shows the time and the last runs (`result`: graded, partial, failed, timeout, locked, offline; `denied`: actions the run was refused). `schedule.py remove --root <learning root>` turns it off.

## Scheduled run

When the prompt says it is a scheduled prepare-ahead run, nobody is there and nothing can be approved. The prompt gives the goal folder (your working folder), the learning root, the skill folder, the notes folder, the Python command, the waiting lessons and today's date. Use those absolute paths, never `~`.

The tick that started you already holds the goal's run lock, and after you finish it commits the notes folder and tells the learner the lesson is ready. So do not lock or unlock, and do not run `ensure`, `nudge`, git, a browser or `open`. The only shell command you may run is `<python> <skill folder>/scripts/serve.py merge ...`, written exactly that way; anything else is refused and logged as a denied action, so read and search files with Read, Glob and Grep, and write with Write and Edit. Those reach only the goal folder, the learning root, the skill folder, the notes folder and the folder of a source file the note names; a search anywhere else, a parent folder included, is refused too.

1. Read the goal's page, `answers.json`, `state.json` and its vault note (in the notes folder, the note whose `page:` line names this goal's `index.html`).
2. Grade the waiting lessons as in Finished lessons first, steps 2 to 4 (Grading on "done", Older pages), then the review answers (Review). A lesson is cleared only by its `data-graded`; the tick checks it.
3. After a pass, write the next lesson as usual, with its review versions, from the source the note names. If the source cannot be read (a web page, or a file you are refused), grade and write the feedback, and leave the next lesson for the next interactive run; say so in the lesson's `.done` line. Never write a lesson from memory in place of the source.
4. Update the vault note as in Grading step 4, without committing. When the prompt says the notes folder is left, do not look for the note: the next interactive run brings it up to date.
5. Never change the profile, even when behavior shows a different preference: leave that for the next interactive run.
6. End with one plain sentence, no list or line breaks: what you graded and what you wrote. The tick logs it.
