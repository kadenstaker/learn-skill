...
- Grading: `ArtifactData` action `query`, collection `answers`, `query.where` on `lesson == <section id>`. Each document has the same fields as a pasted answer: `lesson`, `q`, `kind`, `value`, `correct`, `attempts`, `at`.
